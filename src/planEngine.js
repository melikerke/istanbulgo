...
export const ATTRACTION_TAGS = {
  hagia:       { tags: ["history", "must"], area: "sultanahmet", duration: 90, priority: 10 },
  basilica:    { tags: ["history"],         area: "sultanahmet", duration: 60, priority: 8 },
  topkapi:     { tags: ["history", "must"], area: "sultanahmet", duration: 180, priority: 9 },
  bluemosque:  { tags: ["history", "must"], area: "sultanahmet", duration: 45, priority: 7 },
  grandbazaar: { tags: ["shopping", "must"],area: "sultanahmet", duration: 90, priority: 7 },
  spicebazaar: { tags: ["shopping", "food"],area: "eminonu",     duration: 60, priority: 6 },
  dolma:       { tags: ["history"],         area: "besiktas",    duration: 150, priority: 7 },
  dervish:     { tags: ["history", "views"],area: "various",     duration: 60, priority: 6 },
  galata:      { tags: ["views", "must"],   area: "beyoglu",     duration: 45, priority: 7 },
  maiden:      { tags: ["views"],           area: "uskudar",     duration: 90, priority: 6 },
  camlica:     { tags: ["views"],           area: "uskudar",     duration: 90, priority: 5 },
  pierreloti:  { tags: ["views"],           area: "eyup",        duration: 75, priority: 4 },
  cruise:      { tags: ["water", "must"],   area: "eminonu",     duration: 120, priority: 8 },
  nightcruise: { tags: ["water", "food"],   area: "eminonu",     duration: 180, priority: 5 },
  islands:     { tags: ["water"],           area: "marmara",     duration: 480, priority: 4 },
  hammam:      { tags: ["family"],          area: "various",     duration: 90, priority: 6 },
  foodtour:    { tags: ["food"],            area: "various",     duration: 210, priority: 7 },
  cooking:     { tags: ["food"],            area: "various",     duration: 240, priority: 5 },
  aquarium:    { tags: ["family"],          area: "florya",      duration: 120, priority: 4 },
  hoponoff:    { tags: ["family"],          area: "citywide",    duration: 240, priority: 3 },
  balat:       { tags: ["shopping", "views"], area: "fatih",     duration: 180, priority: 6 },
  taksim:      { tags: ["shopping"],        area: "beyoglu",     duration: 150, priority: 5 },
  ortakoy:     { tags: ["views", "food"],   area: "besiktas",    duration: 90, priority: 5 },
};
...
export function generatePlan({ days, pace, interests }) {
  ...
  let candidates = Object.entries(ATTRACTION_TAGS).map(([id, data]) => {
    ...
    if (data.tags.includes("must") && interests.length > 0) score += 3;
    return { id, ...data, score };
  });

  // Hiç interest seçilmediyse "must"ları öncelikle göster
  if (interests.length === 0) {
    candidates = candidates.filter(c => c.tags.includes("must"));
  }

  // Skor sırasına diz
  candidates.sort((a, b) => b.score - a.score);

  // 2. En yüksek skorlu totalSpots kadarını al
  const selected = candidates.slice(0, totalSpots);
